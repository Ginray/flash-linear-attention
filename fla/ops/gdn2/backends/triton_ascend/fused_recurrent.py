...
"""GDN-2 fused recurrent forward kernel for triton-ascend on Ascend NPU.

Mirrors ``fused_recurrent_gdn2_fwd_kernel`` from
``fla/ops/gdn2/fused_recurrent.py``. Per token the matrix state ``S`` updates as

    S <- Diag(exp(g)) S;  v_new = (w * v) - (b * k)^T S;  S <- S + k v_new^T

with channel-wise gates ``b`` (key axis) and ``w`` (value axis).
"""
...
# Peak fp32 live set: b_h[BK,BV] + b_q,b_k,b_g,b_b[BK] + b_v,b_w,b_o[BV].
# Compiler multi-buffer (~3x analytical peak) is folded into mem_mult.
_RECUR_MEM_MULT = 3.0
_SAFETY_MARGIN = 0.80
_FALLBACK_BV = 32
_MAX_BV = 256
...
def fused_recurrent_gdn2_fwd_kernel_npu(
    ...
    if T_cur > 0:
        ...
        for i_t in tl.range(0, T_cur):
            ...
            b_w = tl.load(p_w, mask=mask_v, other=0).to(tl.float32)
...
def fused_recurrent_gdn2_fwd_npu(
    ...
    **kwargs,
    ...
    for task_off in range(0, task_num, max_tasks):
        ...
        fused_recurrent_gdn2_fwd_kernel_npu[(task_len,)](**kernel_kwargs)
