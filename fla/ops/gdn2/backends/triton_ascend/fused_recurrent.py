...
# triton-ascend exposes extract_slice via cann.extension on some versions.
try:
...
# includes compiler temporaries and multibuffering, calibrated at BK=256.
_RECUR_MEM_MULT = 4.1
_SAFETY_MARGIN = 0.80
_FALLBACK_BV = 32
_MAX_BV = 256
...
def _get_bt(BK: int, BV: int) -> int:
    """Choose the staging width from the UB budget left by the state tile."""
    budget = get_ub_manager().ub_capacity_bytes * _SAFETY_MARGIN
    state = _RECUR_MEM_MULT * BK * BV * 4
    per_token = _RECUR_MEM_MULT * (4 * BK + 2 * BV) * 2
    remaining = budget - state
    if remaining < per_token:
        return 1
    bt = int(remaining // per_token)
    bt = triton.next_power_of_2(bt + 1) // 2
    return max(1, min(_MAX_BT, bt))
...
def fused_recurrent_gdn2_fwd_kernel_npu(
    ...
    task_id = tl.cast(tl.program_id(0), tl.int64) + tl.cast(TASK_OFFSET, tl.int64)
    NV = tl.cdiv(V, BV)
    i_v = task_id % NV
    i_nh = task_id // NV
    ...
    if T_cur > 0:
        ...
        # stage several tokens per DMA while keeping state updates serial.
        for i_t in tl.range(0, T_cur, BT):
            rel = tl.arange(0, BT).to(tl.int64)
            row_valid = (i_t + rel) < T_cur
            m_qk = row_valid[:, None] & mask_k[None, :]
            m_hvv = row_valid[:, None] & mask_v[None, :]

            b_q2 = tl.load(p_q + rel[:, None] * stride_qk, mask=m_qk, other=0)
            b_k2 = tl.load(p_k + rel[:, None] * stride_qk, mask=m_qk, other=0)
            b_g2 = tl.load(p_g + rel[:, None] * stride_hv_k, mask=m_qk, other=0)
            b_b2 = tl.load(p_b + rel[:, None] * stride_hv_k, mask=m_qk, other=0)
            b_v2 = tl.load(p_v + rel[:, None] * stride_hv_v, mask=m_hvv, other=0)
            b_w2 = tl.load(p_w + rel[:, None] * stride_hv_v, mask=m_hvv, other=0)

            # padded tokens must not update the state.
            b_jmax = tl.minimum(BT, T_cur - i_t)
            for j in tl.range(0, b_jmax):
                b_q = tl.reshape(tl.extract_slice(b_q2, [j, 0], [1, BK], [1, 1]), [BK]).to(tl.float32)
                b_k = tl.reshape(tl.extract_slice(b_k2, [j, 0], [1, BK], [1, 1]), [BK]).to(tl.float32)
                b_v = tl.reshape(tl.extract_slice(b_v2, [j, 0], [1, BV], [1, 1]), [BV]).to(tl.float32)

                if USE_QK_L2NORM_IN_KERNEL:
                    b_q = b_q / tl.sqrt(tl.sum(b_q * b_q) + 1e-6)
                    b_k = b_k / tl.sqrt(tl.sum(b_k * b_k) + 1e-6)
                b_q = b_q * scale
                b_g = tl.reshape(tl.extract_slice(b_g2, [j, 0], [1, BK], [1, 1]), [BK]).to(tl.float32)
                ...
                b_b = tl.reshape(tl.extract_slice(b_b2, [j, 0], [1, BK], [1, 1]), [BK]).to(tl.float32)
                b_w = tl.reshape(tl.extract_slice(b_w2, [j, 0], [1, BV], [1, 1]), [BV]).to(tl.float32)
                ...
                tl.store(p_o + tl.cast(j, tl.int64) * stride_hv_v, b_o.to(p_o.dtype.element_ty), mask=mask_v)

                if IS_CONTINUOUS_BATCHING:
                    if INPLACE_FINAL_STATE:
                        state_base = (
                            tl.load(ssm_state_indices + i_n * stride_indices_seq + i_t + j).to(tl.int64)
                            * stride_final_state_token
                        )
                        p_ht = ht + state_base + i_hv * K * V
                    else:
                        p_ht = ht + (bos + i_t + j) * stride_final_state_token + i_hv * K * V
                    if STATE_V_FIRST:
...
def fused_recurrent_gdn2_fwd_npu(
    ...
    **kwargs,
    ...
    # pipeline staged token loads; each program owns its state tile.
    for task_off in range(0, task_num, max_tasks):
        task_len = min(max_tasks, task_num - task_off)
        kernel_kwargs["TASK_OFFSET"] = task_off
        fused_recurrent_gdn2_fwd_kernel_npu[(task_len,)](**kernel_kwargs, multibuffer=True)
