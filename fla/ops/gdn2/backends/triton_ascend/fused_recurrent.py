...
def _npu_pad(n: int, tile: int = _NPU_ALIGN) -> int:
    """Pad a dimension so Ascend leftover DMA cannot reach the next row."""
    n = max(int(n), 1)
    align = max(int(tile), _NPU_ALIGN)
    padded = triton.cdiv(n, align) * align
    if n % _NPU_ALIGN != 0 and n + _NPU_ALIGN > padded:
        padded += align
    return padded
...
def fused_recurrent_gdn2_fwd_kernel_npu(
    ...
    for task_idx in tl.range(core_id, task_num, num_core):
        ...
        if T_cur > 0:
            ...
            p_b = b + base_hv * KP + o_k
            p_w = w + base_hv * VP + o_v
            ...
            for i_t in tl.range(0, T_cur):
                ...
                if IS_CONTINUOUS_BATCHING:
                    if INPLACE_FINAL_STATE:
                        ...
                        p_ht = ht + (bos + i_t64) * stride_final_state_token + i_hv * KS * VS
                    if STATE_V_FIRST:
...
def fused_recurrent_gdn2_fwd_npu(
    ...
    **kwargs,
    ...
    BV = _get_bv(K, V)
    mask_leftover = cu_seqlens is not None or any(
        n % b != 0 for n, b in ((K, BK), (V, BV))
    )

    KP, VP = _npu_pad(K, BK), _npu_pad(V, BV)
    q = _pad_last_dim(q, KP)
    k = _pad_last_dim(k, KP)
    g = _pad_last_dim(g, KP)
    b = _pad_last_dim(b, KP)
    v = _pad_last_dim(v, VP)
    w = _pad_last_dim(w, VP)
    if dt_bias is not None and KP != K:
        dt_bias = _pad_last_dim(dt_bias.reshape(HV, K), KP).reshape(-1)

    KS, VS = _npu_pad(K, BK), _npu_pad(V, BV)
    state_needs_padding = KS != K or VS != V
    h0 = initial_state
    if h0 is not None and state_needs_padding:
        if state_v_first:
            h0 = h0.new_zeros(*h0.shape[:-2], VS, KS)
            h0[..., :V, :K] = initial_state
        else:
            h0 = h0.new_zeros(*h0.shape[:-2], KS, VS)
    ...
    pad_out = VP != V
    out = v.new_zeros(B, T, HV, VP) if user_out is None or pad_out else user_out

    state_writeback = inplace_final_state and state_needs_padding
    if inplace_final_state:
        assert initial_state is not None, "inplace_final_state=True requires an initial_state"
        final_state = h0
    elif output_final_state:
        state_shape = (N, HV, VS, KS) if kernel_v_first else (N, HV, KS, VS)
        if state_needs_padding:
            final_state = q.new_zeros(*state_shape, dtype=torch.float32)
        else:
    ...
    fused_recurrent_gdn2_fwd_kernel_npu[(num_core,)](**kernel_kwargs, multibuffer=False)

    if internal_v_first and final_state is not None:
        final_state = final_state.transpose(-1, -2).contiguous()
    if pad_out:
        logical_out = out[..., :V].contiguous()
        if user_out is None:
            out = logical_out
        else:
            user_out.copy_(logical_out)
            out = user_out
    if state_needs_padding and (state_writeback or output_final_state):
        if state_v_first:
            final_state = final_state[..., :V, :K].contiguous()
        else:
            final_state = final_state[..., :K, :V].contiguous()
        if state_writeback:
